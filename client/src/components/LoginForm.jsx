  ...
  return (
    <div className='min-h-screen flex flex-col md:flex-row'>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 bg-white ">
        ...
        {error && (<div className="mb-6 p-4 bg-rose-50 border border-rose-200 text-rose-700 text-sm rounded-xl flex items-start gap-3">
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}  required placeholder="jhon@example.com"/>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors">
                  {showPassword ? <EyeOffIcon className="w-4 h-4 text-slate-400"/> : <EyeIcon className="w-4 h-4 text-slate-400"/>}
                </button>
              </div>
