...
const PrintPaySlips = () => {
  ...
  useEffect(()=>{
    setPayslips(dummyPayslipData.find((slip)=>slip._id === id))
    setTimeout(()=>{
      setLoading(false)
    },1000)
    ...
    <div className="max-w-2xl mx-auto p-8 bg-white animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div>
          ...
          <p className="font-semibold text-slate-900">{payslips.employee?.firstName} {payslips.employee?.firstName}</p>


        </div>
         <div>
          <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Position </p>
          <p className="font-semibold text-slate-900">{payslips.employee?.firstName} {payslips.employee?.position}</p>
      ...
      <div className="rounded-xl border border-slate-200 overflow-hidden mb-8">
        <table className="w-full text-sm">
          ...
          <tbody>
            <tr className="border-t border-l-slate-100">
             ...
             <tr className="border-t border-l-slate-100">
              ...
              <td className="text-right py-3 px-4 text-slate-900 font-medium">+${payslips.allowances?.toLocaleString()}</td>

            </tr>
             <tr className="border-t border-l-slate-100">
              <td className="py-3 px-4 text-slate-700">Deductions</td>
              <td className="text-right py-3 px-4 text-slate-900 font-medium">-${payslips.deductions?.toLocaleString()}</td>

            </tr>
             <tr className="border-t-2 border-slate-200 bg-slate-50">
              <td className="py-3 px-4 text-slate-700">Net Salary</td>
              <td className="text-right py-4 px-4 text-slate-900 text-lg">-${payslips.netSalary?.toLocaleString()}</td>
